const totFinancingDefault = "0%, 84 months"
const defaultImgUrl = 'images/replacement-parts/furnace.png';

export const furnaceOnly = {
  defaultImgUrl,
  mainTitle: "Furnace Replacement",
  "1.5": {
    productImage: defaultImgUrl,
    productTitle: "1.5 Ton Furnace Only",
    pricingLevels: ["Platinum", "Gold", "Silver"],
    seers: ["18", "16", "14"],
    levelDescriptions: [
      "Highest Efficiency, Total Comfort",
      "Energy Efficient Choice",
      "Better Efficiency, Competitive Price",
    ],
    levelFeatureLists: [
      [
        "",
        "Variable Speed Compressor",
        "Variable speed Indoor Fan",
        "",
      ],
      [
        "",
        "Two Stage Scroll Compressor",
        "80% AFUE Two Stage Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
      [
        "",
        "Single Stage Compressor",
        "80% AFUE Constant Torque Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
    ],
    perMonthPrices: [55, 40, 25],
    levelTotFinancings: [
      totFinancingDefault,
      totFinancingDefault,
      totFinancingDefault,
    ],
    levelTotPrices: [4625, 3325, 2125],
  },
  "2.0": {
    productImage: defaultImgUrl,
    productTitle: "2.0 Ton Furnace Only",
    pricingLevels: ["Platinum", "Gold", "Silver"],
    seers: ["18", "16", "14"],
    levelDescriptions: [
      "Highest Efficiency, Total Comfort",
      "Energy Efficient Choice",
      "Better Efficiency, Competitive Price",
    ],
    levelFeatureLists: [
      [
        "",
        "Variable Speed Compressor",
        "Variable speed Indoor Fan",
        "",
      ],
      [
        "",
        "Two Stage Scroll Compressor",
        "80% AFUE Two Stage Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
      [
        "",
        "Single Stage Compressor",
        "80% AFUE Constant Torque Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
    ],
    perMonthPrices: [57, 42, 27],
    levelTotFinancings: [
      totFinancingDefault,
      totFinancingDefault,
      totFinancingDefault,
    ],
    levelTotPrices: [4799, 3499, 2299],
  },
  "2.5": {
    productImage: defaultImgUrl,
    productTitle: "2.5 Ton Furnace Only",
    pricingLevels: ["Platinum", "Gold", "Silver"],
    seers: ["18", "16", "14"],
    levelDescriptions: [
      "Highest Efficiency, Total Comfort",
      "Energy Efficient Choice",
      "Better Efficiency, Competitive Price",
    ],
    levelFeatureLists: [
      [
        "",
        "Variable Speed Compressor",
        "Variable speed Indoor Fan",
        "",
      ],
      [
        "",
        "Two Stage Scroll Compressor",
        "80% AFUE Two Stage Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
      [
        "",
        "Single Stage Compressor",
        "80% AFUE Constant Torque Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
    ],
    perMonthPrices: [57, 42, 28],
    levelTotFinancings: [
      totFinancingDefault,
      totFinancingDefault,
      totFinancingDefault,
    ],
    levelTotPrices: [4825, 3525, 2325],
  },
  "3.0": {
    productImage: defaultImgUrl,
    productTitle: "3.0 Ton Furnace Only",
    pricingLevels: ["Platinum", "Gold", "Silver"],
    seers: ["18", "16", "14"],
    levelDescriptions: [
      "Highest Efficiency, Total Comfort",
      "Energy Efficient Choice",
      "Better Efficiency, Competitive Price",
    ],
    levelFeatureLists: [
      [
        "",
        "Variable Speed Compressor",
        "Variable speed Indoor Fan",
        "",
      ],
      [
        "",
        "Two Stage Scroll Compressor",
        "80% AFUE Two Stage Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
      [
        "",
        "Single Stage Compressor",
        "80% AFUE Constant Torque Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
    ],
    perMonthPrices: [58, 43, 29],
    levelTotFinancings: [
      totFinancingDefault,
      totFinancingDefault,
      totFinancingDefault,
    ],
    levelTotPrices: [4899, 3599, 2399],
  },
  "3.5": {
    productImage: defaultImgUrl,
    productTitle: "3.5 Ton Furnace Only",
    pricingLevels: ["Platinum", "Gold", "Silver"],
    seers: ["18", "16", "14"],
    levelDescriptions: [
      "Highest Efficiency, Total Comfort",
      "Energy Efficient Choice",
      "Better Efficiency, Competitive Price",
    ],
    levelFeatureLists: [
      [
        "",
        "Variable Speed Compressor",
        "Variable speed Indoor Fan",
        "",
      ],
      [
        "",
        "Two Stage Scroll Compressor",
        "80% AFUE Two Stage Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
      [
        "",
        "Single Stage Compressor",
        "80% AFUE Constant Torque Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
    ],
    perMonthPrices: [54, 44, 30],
    levelTotFinancings: [
      totFinancingDefault,
      totFinancingDefault,
      totFinancingDefault,
    ],
    levelTotPrices: [4499, 3699, 2499],
  },
  "4.0": {
    productImage: defaultImgUrl,
    productTitle: "4.0 Ton Furnace Only",
    pricingLevels: ["Platinum", "Gold", "Silver"],
    seers: ["18", "16", "14"],
    levelDescriptions: [
      "Highest Efficiency, Total Comfort",
      "Energy Efficient Choice",
      "Better Efficiency, Competitive Price",
    ],
    levelFeatureLists: [
      [
        "",
        "Variable Speed Compressor",
        "Variable speed Indoor Fan",
        "",
      ],
      [
        "",
        "Two Stage Scroll Compressor",
        "80% AFUE Two Stage Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
      [
        "",
        "Single Stage Compressor",
        "80% AFUE Constant Torque Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
    ],
    perMonthPrices: [61, 45, 31],
    levelTotFinancings: [
      totFinancingDefault,
      totFinancingDefault,
      totFinancingDefault,
    ],
    levelTotPrices: [5099, 3799, 2599],
  },
  "5.0": {
    productImage: defaultImgUrl,
    productTitle: "5.0 Ton Furnace Only",
    pricingLevels: ["Platinum", "Gold", "Silver"],
    seers: ["18", "16", "14"],
    levelDescriptions: [
      "Highest Efficiency, Total Comfort",
      "Energy Efficient Choice",
      "Better Efficiency, Competitive Price",
    ],
    levelFeatureLists: [
      [
        "",
        "Variable Speed Compressor",
        "Variable speed Indoor Fan",
        "",
      ],
      [
        "",
        "Two Stage Scroll Compressor",
        "80% AFUE Two Stage Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
      [
        "",
        "Single Stage Compressor",
        "80% AFUE Constant Torque Furnace",
        "10 Year Parts / 1 Year Labor Warranty",
      ],
    ],
    perMonthPrices: [61, 46, 31],
    levelTotFinancings: [
      totFinancingDefault,
      totFinancingDefault,
      totFinancingDefault,
    ],
    levelTotPrices: [5125, 3825, 2625],
  },
}